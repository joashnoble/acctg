...
<div v-show="showEntry === false" class="animated fadeIn">
      <b-card border-variant="default" class="text-left card-header-primary">
        ...
        <b-row class="mb-2">
            ...
            <b-col sm="4">
                <b-input-group prepend-html='<i class="fa fa-search"></i> &nbsp; Search'>
                    <b-form-input 
                                v-model="filters.cashreceipts.criteria"
                                type="text" 
                                placeholder="Search"
                                debounce="250">
                    </b-form-input>
        ...
        <b-row mb="2">
            ...
            :fields="tables.cashreceipts.fields"
            :items.sync="tables.cashreceipts.items"
            :filter="filters.cashreceipts.criteria"
            :current-page="paginations.cashreceipts.currentPage"
            :per-page="paginations.cashreceipts.perPage"
            @filtered="onFiltered($event,'cashreceipts')"
            >
                <template v-slot:cell(row_data)="row">
                    <i
                    @click.stop="row.toggleDetails(),CashReceiptDetails(row) " 
                        ...
                        <b-col sm="12" class="mt-1 mb-1">
                            <table  width="100%" class="background-white table-info" >
                                <tr>
                                    ...
                                    <td width="15%" class="bold">Official Receipt: </td>
                                    <td width="35%" >{{ row.item.or_no }}</td>
                                </tr>
                                <tr>
        ...
        <template v-slot:footer> 
            <b-row mb="0">
                <b-col sm="12" class="my-1 mb-0" >
                    <b-pagination mb="0"
                    v-model="paginations.cashreceipts.currentPage"
                    :total-rows="paginations.cashreceipts.totalRows"
                    :per-page="paginations.cashreceipts.perPage"
                    prev-text="Previous"
                    next-text="Next"
...
<div v-show="showEntry" class="animated fadeIn">
    <b-card border-variant="default" class="text-left card-header-primary">
        ...
        <b-form autocomplete="off">
            <b-row>
                <b-col sm="3">
                        ...
                        <date-picker  style="width:100%"
                            v-model="forms.cashreceipt.fields.date_txn" 
                            lang="en" 
                ...
                <b-col sm="3">
                    <b-form-group>
                        ...
                        <b-form-input
                            ref="txn_no"
                            id="txn_no"
                            v-model="forms.cashreceipt.fields.txn_no"
                            debounce="250"
                            type="text"
                ...
                <b-col sm="4">
                    <b-form-group>
                        ...
                        <select2
                            ...
                            v-model="forms.cashreceipt.fields.payment_method_id"
                        >
            ...
            <b-row>
                <b-col sm="3">
                    <b-form-group>
                        ...
                        <select2
                            ...
                            v-model="forms.cashreceipt.fields.department_id"
                        >
                ...
                <b-col sm="3">
                    <b-form-group>
                        ...
                        <select2
                            ...
                            v-model="forms.cashreceipt.fields.bank_id"
                        >
                ...
                <b-col sm="2">
                    <label >Date</label>
                    <date-picker  style="width:100%"
                        v-model="forms.cashreceipt.fields.check_date" 
                        lang="en" 
                        input-class="form-control mx-input"
                        format="MMMM DD, YYYY"
                        :clearable="false">
                ...
                <b-col sm="2">
                    <b-form-group>
                        ...
                        <b-form-input
                            v-model="forms.cashreceipt.fields.check_no"
                            debounce="250"
                            type="text"
                        >
                        </b-form-input>
            ...
            <b-row>
                ...
                <b-form-group>
                    ...
                    <select2
                        ...
                        v-model="forms.cashreceipt.fields.particular_id"
                    >
                ...
                <b-col sm="2">
                    <b-form-group>
                        ...
                        <b-form-input
                            v-model="forms.cashreceipt.fields.or_no"
                            debounce="250"
                            type="text"
                        >
                        </b-form-input>
                ...
                <b-col sm="2">
                    ...
                    <vue-autonumeric 
                        ...
                        v-model="forms.cashreceipt.fields.amount" 
            ...
            <b-row>
                <b-col sm="12">
                    <b-form-group>
                        <label>Remarks </label>
                        <b-form-textarea
                            v-model="forms.cashreceipt.fields.remarks"
                            debounce="250"
                            type="text"
                            rows="2"
                            no-resize
        ...
        <template v-slot:footer>
            <b-row class="">
                <b-col sm="12">
                    <b-button 
                        :disabled="forms.cashreceipt.isSaving" 
                        variant="success" 
                        @click="onCashReceiptEntry">
                        <b-spinner v-if="forms.cashreceipt.isSaving" small variant="light" label="Spinning"></b-spinner>
    ...
    <canceljournal entity="cashreceipt" table="cashreceipts" primary_key="journal_id" ref="canceljournal"></canceljournal>
</div>
</template>
...
export default {
    ...
    created(){
        this.fillTableList('cashreceipts');
    ...
    methods:{
        async CashReceiptDetails(row){
            if(row.detailsShowing == true){
                return
            }
            await this.$http.get('/api/cashreceipt_details/' + row.item.journal_id, {
                headers: {
        ...
        clearJournal(){
            this.clearFields('cashreceipt')
            this.forms.cashreceipt.fields.date_txn = new Date()
            this.forms.cashreceipt.fields.check_date = ''
        ...
        async onCashReceiptEntry(){
            if(this.forms.cashreceipt.fields.date_txn == '' || this.forms.cashreceipt.fields.date_txn ==  null){
                this.$notify({ type: 'error', group: 'notification', title: '<b>Date Error!</b>', text: 'Please make sure transaction date is valid!' })
            }else {
                var checkDate = { 'date_txn': this.forms.cashreceipt.fields.date_txn}
                if(await this.CheckJournalEntries()){ 
                    if( await this.checkPeriod(checkDate) == true){ // MEANS period is not accepted
                        ...
                        this.forms.cashreceipt.fields.journalentry = this.tables.journalentries.items;
                        await this.createEntity('cashreceipt', false, 'cashreceipts')
                    }
                }
            }
        },
